...
def writeFASTAfromJson():
	...
	if os.path.isfile(working_directory+"proteindb.fsa") == False:
		with open(working_directory+"card.json") as json_file:
			json_data = json.load(json_file)
			with open (working_directory+'proteindb.fsa', 'w') as wp:
...
def makeBlastDB(inType, inputSeq):
	if os.path.isfile(working_directory+"proteindb.fsa") == True and os.path.exists(working_directory+"proteindb.fsa") == True  and os.path.exists(working_directory+"protein.db.phr") == True and os.path.exists(working_directory+"protein.db.pin") == True and os.path.exists(working_directory+"protein.db.psq") == True :
		print "DB exists"
	else:
		print "create DB."
		os.system('makeblastdb -in '+working_directory+'proteindb.fsa -dbtype prot -out '+working_directory+'protein.db')
...
def runBlast(inType, inputSeq, threads, outputFile, criteria, data_type, clean, orf):	
	...
	if inType == 'contig':
		...
		if os.stat(working_directory+"/"+file_name+".contig.fsa").st_size != 0:
			...
			blastCLine = NcbiblastpCommandline(query=working_directory+"/"+file_name+".contig.fsa", db=working_directory+"protein.db", outfmt=5, out=working_directory+"/"+file_name+".blastRes.xml",num_threads=threads)
		...
		logging.info("runBlast => start blastP for inType: " + inType)
		from Bio.Blast.Applications import NcbiblastpCommandline
		blastCLine = NcbiblastpCommandline(query=inputSeq, db=working_directory+"protein.db", outfmt=5, out=working_directory+"/"+file_name+".blastRes.xml",num_threads=threads)
		stdt, stdr = blastCLine()
		result_handle = open(working_directory+"/"+file_name+".blastRes.xml")
		startBlast = True
		...
		blastCLine = NcbiblastxCommandline(query=working_directory+"/"+file_name+".read.fsa", db=working_directory+"protein.db", outfmt=5, out=working_directory+"/"+file_name+".blastRes.xml",num_threads=threads)
		stdt, stdr = blastCLine()
		result_handle = open(working_directory+"/"+file_name+".blastRes.xml")
		startBlast = True
