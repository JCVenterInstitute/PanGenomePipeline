...
while ($line = <DEF>)                                    													 
{
	@split_line = split(/\t/, $line);															#splits up the prop def file lines by tab 				#in an array attached to the property name, and in a temp array for the line
...
open (STEP, "$prop_step");
while ($line = <STEP>)																			#go through step table line by line
{

	@split_line = split(/\t/, $line);															#split up the line of step table by tab
...
while ($line = <EV>)																			#go through it line by line
{
	@split_line = split(/\t/, $line);															#split the line up by tabs
...
sub try_property
{
	...
	($answered, $result) = evaluate_property($acc_id);      # <----- changed from 2 parameters to 1
	if ($answered)
	{
		$results{$acc} = $result;
		print_result($acc);
