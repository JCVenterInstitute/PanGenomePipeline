...
my $help_text = "This program reads in a 3 column tab delimitted file from standard input. The 3 columns are the contig name, the contig position, and the coverage depth - missing contig positions are assumed to have coverage 0. This program outputs to standard output a 3 column tab delimitted file: column 1 is contig name, column 2 is start of 0 coverage region, and column 3 is stop of 0 coverage region - final row for a contig will have a stop of end representing the end coordinate of the contig which is unknown to the program.
