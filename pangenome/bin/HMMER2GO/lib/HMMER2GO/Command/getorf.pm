...
sub opt_spec {
    ...
    [ "table=i",       "Translation table, as numbered by NCBI"                 ],
...
sub execute {
    ...
    my $result = _run_getorf($getorf, $infile, $outfile, $find, $orflen, 
			     $allorfs, $nomet, $sense, $table, $verbose);
}

sub _run_getorf {
    my ($getorf, $infile, $outfile, $find, 
	$orflen, $allorfs, $nomet, $sense, $table, $verbose) = @_;
	...
	my $orffile = _getorf($getorf, $iname, $isuffix, $fcount, 
			      $id, $seq, $find, $nomet, $table, $orflen);
...
sub _getorf {
	...
	$id, $seq, $find, $nomet, $table, $orflen) = @_;
