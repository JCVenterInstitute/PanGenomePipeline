...
sub _map_go_terms {
	...
	chomp $mapping;
	next if $mapping =~ /^!/;
    # Changed the next two lines to allow other than 'Pfam:' at the start of the string:
	if ($mapping =~ /(\S+):(\S+) (\S+ \> )(GO\:\S+.*\;) (GO\:\d+)/) {
	    my ($lexicon, $pf, $pf_name, $pf_desc, $go_term) = ($1, $2, $3, $4, $5);
	    $pf_name =~ s/\s.*//;
	    $pf_desc =~ s/\s\;//;
